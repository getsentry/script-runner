...
function mergeRegionKeys(data: unknown, regions: string[]) {
  try {
    if (typeof data === "object" && data !== null) {
      const shouldMerge = Object.keys(data).every((r: string) =>
        regions.includes(r)
      );
      if (shouldMerge) {
        ...
        if (
          !firstRegionData.every((el) => typeof el === "object" && el !== null)
        ) {
          return null;
        }
        ...
        for (let i = 1; i < firstRegionData.length; i++) {
          ...
          if (
            ...
            !rowKeys.every((k) => firstRowKeys.includes(k))
          ) {
            return null;
          }
        }
...
function getGridData(
  data: { [region: string]: unknown[] } | unknown,
  regions: string[]
) {
  ...
  if (
    ...
    mergedData.every((row) => typeof row === "object" && row !== null)
  ) {
  ...
  try {
    ...
    if (merged && Array.isArray(merged)) {
      if (DATE_KEY in merged[0]) {
        const numericFields = Object.entries(merged[0])
        ...
        const mergedByDate: object = merged.reduce(
          (
            acc: { [date: string]: { [key: string]: unknown } },
            curr: { date: string; [key: string]: unknown }
          ) => {
...
function ScriptResult(props: Props) {
  ...
  function applyJqFilter(raw: RunResult | null, filter: string) {
    ...
    jq.then((jq: any) => jq.json(raw, filter))
      .catch(() => {
  ...
  return (
    <div className="function-result">
      ...
      <div className="function-result-filter">
        <input
          type="text"
          placeholder="Filter results with jq"
